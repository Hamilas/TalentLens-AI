...
class AppSettings(BaseSettings):
    ...
    APP_NAME: str = Field(default="TalentLens-AI", description="Application name")
...
class DatabaseSettings(BaseSettings):
    ...
    POSTGRES_DB: str = Field(
        default="talentlens_db", description="PostgreSQL database name"
    )
    ...
    POSTGRES_TEST_DB: str = Field(
        default="talentlens_test_db", description="PostgreSQL test database name"
    )
...
class LanguageModelSettings(BaseSettings):
    ...
    DEFAULT_MODEL: AllModelEnum = Field(
        default=GroqModelName.LLAMA_31_8B, description="Default language model"
    )
    ...
    def available_models(self) -> set:
        ...
        if self.GROQ_API_KEY:
            models.update(set(GroqModelName))
